...
export default function AdminReportsPage() {
  ...
  const fetchTickets = useCallback(async () => {
    setLoading(true);
    const params = new URLSearchParams({ scope, category: "ABUSE" });
    ...
    <div className="mx-auto max-w-5xl space-y-6 p-6">
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border border-[var(--border-color)] bg-[var(--surface)] p-12 text-center">
          <p className="text-[var(--muted-text)]">No abuse reports found</p>
        </div>
      ) : (
        <div className="space-y-3">
          {tickets.map((ticket) => (
            <div
              key={ticket.id}
              className="rounded-xl border border-[var(--border-color)] bg-[var(--surface)] p-5 transition-colors hover:border-[var(--alert-red)]/30"
            >
